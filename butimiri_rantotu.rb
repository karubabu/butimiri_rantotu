		...
		ms.each do |m|
			if m.message.to_me?()
				if !m.message.from_me?()
					if m.message.to_s =~ /ﾌﾞﾘ|ﾘｭﾘｭ|ﾌﾞﾂ|ﾁﾁ|ﾐﾘ|㍉|ﾌﾞﾁ|ぶちっ|みりっ|([ぶちみりブチミリ]){4,}/ and m[:created] > DEFINED_TIME and !m.retweet? and m.message.to_s !~ /[\(（]@karubabu\s?[\)）]/ then
					...
					elsif m.message.to_s =~ /([うおあｕｏａio]){5,}?|[!！]{10,}/ and m[:created] > DEFINED_TIME and !m.retweet? then
